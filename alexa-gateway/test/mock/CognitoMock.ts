...
import { vestibuleClientId } from './IotDataMock';
...
export async function generateValidToken() {
    return await generateToken(await getSharedKey(), vestibuleClientId, authenticationProps.testClientIds[0], new Date(Date.now() + 5000), authenticationProps.testPoolId, authenticationProps.testRegionId);
}

export async function generateValidScope(): Promise<Message.Scope> {
